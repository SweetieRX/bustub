...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    auto prev = &root_;
    for (auto c : key) {
      if (prev->get()->HasChild(c)) {
        prev = prev->get()->GetChildNode(c);
      } else {
        prev = prev->get()->InsertChildNode(c, std::make_unique<TrieNode>(c));
      }
    }
    if (prev->get()->IsEndNode()) {
      latch_.WUnlock();
      return false;
    }
    *prev = std::make_unique<TrieNodeWithValue<T>>(std::move(**prev), value);
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    for (auto c : key) {
      ...
      st.push(std::make_tuple(c, prev));
      prev = prev->get()->GetChildNode(c);
    }
    prev->get()->SetEndNode(false);
    while (!st.empty()) {
      auto value = st.top();
      st.pop();
      auto k = std::get<0>(value);
      auto node = std::get<1>(value);
      auto next = node->get()->GetChildNode(k);
      if (next != nullptr && !next->get()->HasChildren()) {
        node->get()->RemoveChildNode(k);
      }
    }
    latch_.WUnlock();
    return true;
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    auto flag_node = dynamic_cast<TrieNodeWithValue<T> *>(prev->get());
    if (!flag_node) {
      latch_.RUnlock();
      return {};
    }
